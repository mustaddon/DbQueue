...
namespace DbQueue.Abstractions
{
    public interface IDbQueue
    {
        ...
        IAsyncEnumerable<byte[]> Peek(string queue, long index = 0, CancellationToken cancellationToken = default);
        IAsyncEnumerable<byte[]> Pop(string queue, CancellationToken cancellationToken = default);
